...
async function viewPurchasedTicket(user: User) {
...
async function editEvent() {
    ...
    if (eventDetails.title) event.title = eventDetails.title;
    if (eventDetails.date) event.date = eventDetails.date;
    if (eventDetails.time) event.time = eventDetails.time;
    if (eventDetails.city) event.city = eventDetails.city;
    if (eventDetails.ticketStock !== undefined) event.availableTickets = eventDetails.ticketStock;
...
async function viewEvents(adminView: boolean) {
    const availableEvents = events.filter(event => event.availableTickets > 0);
...
async function viewPurchasedTickets(user: User) {
    if (user.purchasedTickets.length === 0) {
        console.log(chalk.magenta("You have not purchased any tickets yet."));
    } else {
        console.log(chalk.green("Your purchased tickets:"));
